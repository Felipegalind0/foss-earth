import { Mesh, VertexData, NullEngine, Scene } from "@babylonjs/core";
...
function calibrateCpuRepeats(query, rays, output) {
  // A file URL is not cross-origin isolated, so Chromium rounds performance.now.
  // Measure at least 4 ms of hot-loop work to avoid reporting cheap queries as 0.
  let repeats;
  // Repeat calibration once so first-use JIT work cannot pick a tiny count.
  for (let calibration = 0; calibration < 2; calibration++) {
    repeats = 1;
    while (true) {
      const start = performance.now();
      for (let i = 0; i < repeats; i++) query(rays, output);
      if (performance.now() - start >= 4 || repeats >= 1048576) break;
      repeats *= 2;
    }
  }
  return repeats;
...
function assertMatches(expected, actual, label) {
  for (let i = 0; i < expected.length; i += 4) {
    if (!Number.isFinite(expected[i]) || !Number.isFinite(actual[i])) throw new Error(`${label}: non-finite distance for ray ${i / 4}`);
...
async function run() {
  ...
  let device, production, gpu, gpuBrute;
  const report = { generatedAt: new Date().toISOString(), source: __BENCHMARK_SOURCE__, buildMachine: __BUILD_MACHINE__,
    environment: { userAgent: navigator.userAgent, secureContext: isSecureContext, crossOriginIsolated, hardwareConcurrency: navigator.hardwareConcurrency },
    method: { rounds, warmupBatches: warmup, rayCounts: [1, 5, 128, 1024], alternatingOrder: true,
      ...
      timing: "End-to-end wall latency per batch. CPU repetition count is calibrated per backend and case by doubling until >=4 ms total, capped at 1,048,576 repeats (calibration performed twice to warm JIT); same count used throughout a case and reported below. CPU totals divided by repeats; GPU always timed individually including readback. P95 across 15 samples (maximum order statistic). Browser file URL is not cross-origin isolated; GPU samples are timer-quantized. No GPU timestamp or CPU energy claim.",
      algorithms: "Production Babylon finite-ray picking; custom CPU and WebGPU Moller-Trumbore triangle queries with either one whole-mesh AABB followed by a linear scan (brute force) or a median-split BVH (8 triangles per leaf). CPU and GPU prototypes share identical packed geometry, ray data, tolerances and traversal order.",
    ...
    limitations: "Synthetic benchmark in an idle browser, no renderer or streaming workload. Prototype implementations are experimental query kernels, not integrated collision backends; production query includes transforms/geodetic metadata omitted by the prototypes. CPU builds the BVH once; GPU copies it, with build/upload/compile measured separately. Float32 local coordinates required. Brute GPU uses one invocation per ray, not a parallel triangle reduction. Does not move JSBSim's CPU WASM contact solver to GPU. No game FPS, battery, watts, or cross-device claims. Repeated CPU batches are hot-cache throughput divided by repetitions; compare sub-timer-resolution values cautiously.",
    preparation: {}, results: [] };
  ...
  try {
    ...
    started = performance.now(); const brute = buildBvh(geometry.positions, geometry.indices, geometry.indices.length / 3);
    report.preparation.cpuBrutePackingMs = performance.now() - started;
    const cpu = createCpuBvhQuery(bvh);
    const cpuBrute = createCpuBvhQuery(brute);
    ...
    for (const scenario of ["dense-local-hit", "distant-miss"]) for (const count of report.method.rayCounts) {
      ...
      started = performance.now(); gpuBrute = await createGpuBvhQuery(device, pipeline, brute, count);
      const gpuBruteUploadAndAllocationMs = performance.now() - started;
      const timings = { productionCpu: [], cpuBrute: [], cpuBvh: [], gpuBrute: [], gpuBvh: [] };
      const output = new Float32Array(count * 4), reference = new Float32Array(count * 4), cpuOutput = new Float32Array(count * 4);
      const calibrationRays = makeRays(count, scenario, 54321);
      const repeatsPerSample = { productionCpu: calibrateCpuRepeats(production.query, calibrationRays, output), cpuBrute: calibrateCpuRepeats(cpuBrute, calibrationRays, output), cpuBvh: calibrateCpuRepeats(cpu, calibrationRays, output), gpuBrute: 1, gpuBvh: 1 };
      ...
      for (let round = -warmup; round < rounds; round++) {
        ...
        cpuBrute(rays, output); assertMatches(reference, output, "Production vs CPU brute force");
        await gpuBrute.query(rays, output); assertMatches(reference, output, "Production vs GPU brute force");
        validatedRays += count;
        const order = round % 2 ? ["gpuBvh", "gpuBrute", "cpuBvh", "cpuBrute", "productionCpu"] : ["productionCpu", "cpuBrute", "cpuBvh", "gpuBrute", "gpuBvh"];
        for (const backend of order) {
          const repeats = repeatsPerSample[backend];
          const start = performance.now();
          for (let repeat = 0; repeat < repeats; repeat++) {
            if (backend === "gpuBvh") await gpu.query(rays, output);
            else if (backend === "gpuBrute") await gpuBrute.query(rays, output);
            else if (backend === "cpuBvh") cpu(rays, output);
            else if (backend === "cpuBrute") cpuBrute(rays, output);
      ...
      report.results.push({ scenario, rays: count, triangles: bvh.triangles, gpuUploadAndAllocationMs, gpuBruteUploadAndAllocationMs, repeatsPerSample, validatedRays, checksum, timing,
      ...
      gpu.dispose(); gpu = null; gpuBrute.dispose(); gpuBrute = null;
      details.textContent = JSON.stringify(report, null, 2);
      console.info(`collision-benchmark: ${scenario} / ${count} rays complete`);
    }
    report.completedAt = new Date().toISOString();
    report.backgroundedDuringRun ??= false;
    document.querySelector("#summary").innerHTML = `<table><tr><th>Scenario / rays</th><th>Production CPU ms</th><th>CPU brute ms</th><th>CPU BVH ms</th><th>GPU brute + readback ms</th><th>GPU BVH + readback ms</th></tr>${report.results.map(r => `<tr><td>${r.scenario} / ${r.rays}</td>${["productionCpu", "cpuBrute", "cpuBvh", "gpuBrute", "gpuBvh"].map(name => `<td>${r.timing[name].medianMs.toFixed(4)}</td>`).join("")}</tr>`).join("")}</table>`;
    ...
    document.removeEventListener("visibilitychange", onHidden); gpu?.dispose(); gpuBrute?.dispose(); production?.dispose(); device?.destroy(); runButton.disabled = false;
  }
}
runButton.addEventListener("click", () => { run().catch(console.error); });
window.runCollisionBenchmark = run;
